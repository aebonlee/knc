import{s,T as c}from"./index-2EfPpdxR.js";async function h({userId:r,type:e,title:t,message:n,link:i}){if(s)try{await s.from(c.notifications).insert({user_id:r,type:e,title:t,message:n,link:i||null})}catch(a){console.error("알림 생성 실패:",a)}}async function g(r,e){if(!(!s||r.length===0))try{const t=r.map(n=>({user_id:n,type:e.type,title:e.title,message:e.message,link:e.link||null}));await s.from(c.notifications).insert(t)}catch(t){console.error("일괄 알림 생성 실패:",t)}}async function f(r){if(!s)return{success:!1,error:"Supabase 초기화 실패"};try{const{data:e,error:t}=await s.functions.invoke("send-email",{body:r});if(t)throw t;if(e?.error)throw new Error(e.error);return{success:!0}}catch(e){const t=e instanceof Error?e.message:String(e);return console.error("[notifications] sendEmail 오류:",t),{success:!1,error:t}}}async function m(r){if(!s)return{success:!1,error:"Supabase 초기화 실패"};try{const{data:e,error:t}=await s.functions.invoke("send-sms",{body:r});if(t)throw t;if(e?.error)throw new Error(e.error);if(!e?.success)throw new Error(e?.message||"SMS 발송 실패");return{success:!0}}catch(e){const t=e instanceof Error?e.message:String(e);return console.error("[notifications] sendSMS 오류:",t),{success:!1,error:t}}}function u(r){const{title:e,body:t}=r,n="산업안전",i="https://knc.dreamitbiz.com",a="#0F2B5B";return`<!DOCTYPE html>
<html lang="ko">
